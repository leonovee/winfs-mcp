...
**Диагностика (v0.9, #3).** Чтобы поймать зависание с данными, есть опциональное логирование транспорта: переменная окружения `WINFS_TRANSPORT_LOG=<путь>` в блоке `env` записи winfs в `claude_desktop_config.json`. По умолчанию выключено (ноль накладных расходов). Пишет только метаданные (время, id, метод, размер в байтах, длительность) — **не тела** запросов/ответов, без содержимого файлов и секретов. Сопоставляется с собственным логом Claude Desktop (`%APPDATA%\Claude\logs\mcp-server-winfs.log`) через `node scripts/analyze-transport-hang.mjs <winfs-log> <cd-log>` — показывает, на каком участке транспорта застряло. Полный протокол: `audit/investigations/v0.9-transport-hang.md`. Вывод расследования: винить, скорее всего, не winfs (impl быстрый — подтверждено 3 раза), а транспорт Claude Desktop; итог — отчёт в Anthropic.
